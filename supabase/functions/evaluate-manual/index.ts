...
serve(async (req) => {
  ...
  try {
    ...
    console.log('\n🔬 Running preflight sanity check...');
    const preflightQuestion = "Which battery fixes CMOS FAIL and on what page?";
    const expectedTerm = "CR-2032";
    
    // Search for preflight question
    const preflightSearchResponse = await fetch(`${supabaseUrl}/functions/v1/search-manuals-robust`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        query: preflightQuestion,
        manual_id,
        max_results: 8
      })
    });

    if (!preflightSearchResponse.ok) {
      throw new Error('Preflight search failed - RAG pipeline may be broken');
    }

    const preflightSearchData = await preflightSearchResponse.json();
    const preflightPassages = preflightSearchData.results || [];
    
    // Assert: At least one chunk contains expected term
    const hasExpectedTerm = preflightPassages.some((p: any) => 
      p.content.toUpperCase().includes(expectedTerm.toUpperCase())
    );
    
    if (!hasExpectedTerm) {
      throw new Error(`❌ PREFLIGHT FAILED: No retrieved chunks contain "${expectedTerm}". Retrieved ${preflightPassages.length} chunks. RAG retrieval may be broken.`);
    }
    
    console.log(`✅ Preflight retrieval OK: Found "${expectedTerm}" in ${preflightPassages.filter((p: any) => p.content.toUpperCase().includes(expectedTerm.toUpperCase())).length} chunks`);
    
    // Generate answer for preflight
    const preflightPassagesText = preflightPassages
      .map((p: any) => `[Page ${p.page_start}] ${p.content}`)
      .join('\n\n---\n\n');

    const preflightAnswerResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: `You are Lead Arcade Technician. Answer using ONLY the provided passages from the arcade manual.
...
Return JSON with this exact structure:
{
  ...
  "coverage": "strong|medium|weak"
          ...
          { 
            role: 'user', 
            content: `Question: ${preflightQuestion}

Passages (verbatim from manual):
${preflightPassagesText.substring(0, 8000)}` 
          }
        ],
    ...
    if (!preflightAnswerResponse.ok) {
      throw new Error('Preflight answer generation failed');
    }

    const preflightAnswerData = await preflightAnswerResponse.json();
    const preflightAnswer = JSON.parse(preflightAnswerData.choices[0].message.content);
    
    // Assert: Final JSON includes sources with the expected page
    const expectedPage = preflightPassages.find((p: any) => 
      p.content.toUpperCase().includes(expectedTerm.toUpperCase())
    )?.page_start;
    
    const hasCitation = preflightAnswer.citations?.some((c: any) => c.page === expectedPage);
    
    if (!hasCitation) {
      console.warn(`⚠️ PREFLIGHT WARNING: Answer doesn't cite page ${expectedPage} containing "${expectedTerm}"`);
      console.warn(`Answer citations:`, JSON.stringify(preflightAnswer.citations));
      console.warn(`Answer text:`, preflightAnswer.answer);
      throw new Error(`❌ PREFLIGHT FAILED: Generated answer doesn't cite page ${expectedPage} where "${expectedTerm}" appears. Citation quality may be broken.`);
    }
    
    console.log(`✅ Preflight citation OK: Answer cites page ${expectedPage}`);
    console.log(`✅ PREFLIGHT PASSED - RAG pipeline is working\n`);
    // ===== END PREFLIGHT =====
