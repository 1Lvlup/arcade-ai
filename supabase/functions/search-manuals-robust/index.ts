...
import { withErrorHandler, handleOpenAIError } from '../_shared/errorHandler.ts';

const FUNCTION_NAME = 'search-manuals-robust';
...
serve(withErrorHandler(FUNCTION_NAME, async (req) => {
  ...
  if (authHeader) {
    try {
      ...
      console.log('JWT auth failed, continuing without tenant context');
    }
  }
