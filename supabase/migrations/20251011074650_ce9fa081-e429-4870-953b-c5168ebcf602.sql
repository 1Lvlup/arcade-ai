-- =============================================
-- PHASE 1: Manual Metadata System (Fixed)
-- =============================================
...
-- 4. Create upsert_manual_metadata RPC (FIXED - renamed return columns)
CREATE OR REPLACE FUNCTION public.upsert_manual_metadata(p_metadata JSONB)
RETURNS TABLE(result_manual_id TEXT, result_canonical_slug TEXT, result_status TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
  ...
  RETURN QUERY SELECT v_manual_id, v_canonical_slug, 'success'::TEXT;
END;
$$;
...
BEGIN
  ...
  WHERE manual_metadata.manual_id = p_manual_id;
...
SECURITY DEFINER
AS $$
  SELECT manual_metadata.manual_id, manual_metadata.canonical_title, manual_metadata.canonical_slug, manual_metadata.platform, manual_metadata.doc_type
  FROM public.manual_metadata
  ORDER BY manual_metadata.canonical_title;
$$;
...
-- 11. Run backfill for seeded manuals
DO $$
DECLARE
